module HTSBenchmark
  ...
  input :skip_normal, :boolean, "Don't simulate the normal sample", false
  task :NEAT_genreads => :text do |somatic,germline,reference,normal_depth,tumor_depth,haploid,skip_normal|
    input = file('input')
    Open.mkdir input
    ...
    CMD.cmd("cat #{input.somatic} > #{input.joined}", :nofail => true)
    CMD.cmd("cat #{input.germline} |grep -v '#' >> #{input.joined}", :nofail => true)
    ...
    CMD.cmd_log("gen_reads.py", "-c #{tumor_depth} -r '#{reference_gunzip}' -p #{ploidy} -M 0 -R 100 --pe 100 10 -o '#{output.tumor}' -v '#{input.joined}' --bam --vcf")
    bam = output['tumor_golden.bam']
    CMD.cmd("samtools addreplacerg  -o #{bam}.tmp -R 'NEAT' #{bam}  ; samtools view -b -S #{bam}.tmp > #{bam}; rm #{bam}.tmp")

    if ! skip_normal
      CMD.cmd_log("gen_reads.py", "-c #{normal_depth} -r '#{reference_gunzip}' -p #{ploidy} -M 0 -R 100 --pe 100 10 -o '#{output.normal}' -v '#{input.germline}' --bam --vcf")
